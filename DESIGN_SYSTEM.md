...
| `client-patrimoine` | Onglet Patrimoine : vue consolidée + saisie en funnel + liste des avoirs groupée par classe d'actif. Édition / suppression | table `client_assets` (RLS cabinet) ; config `lib/client/patrimoine.config.ts` |
| `client-patrimoine-funnel` | Saisie d'un avoir **piloté par la classe d'actif** : (1) grille des classes (immobilier, actions & fonds, PE non coté, épargne, crypto, autres), (2) champs contextuels propres à la classe — enveloppe fiscale, type de support, zone géographique | évite les combinaisons absurdes ; géo déclarée ⇒ consolidation exacte |
